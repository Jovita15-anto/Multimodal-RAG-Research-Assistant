        ...
        if file_type == "text/plain":
            ...
            for page_number, page in enumerate(pdf_reader.pages, start=1):
                ...
                if text.strip():
                    ...
                    documents.append(document)    
            print(f"[{file_name}] PDF pages: {len(pdf_reader.pages)}, pages with extracted text: {sum(1 for d in documents if d.metadata.get('source')==file_name)}")
...
def create_uploaded_knowledge_base(uploaded_files):
    ...
    vector_store.add_documents(uploaded_chunks)   # replaces the Chroma.from_documents(...) call
    return vector_store